...
class PromptChainingAgent:
    ...
    def run(self, raw_text: str) -> dict:
        ...
        clean_text = call_llm("You are a text cleaning assistant.", preprocess_prompt, json_output=False)
        if not clean_text:
        ...
        classification = call_llm("You are a text classification specialist.", classify_prompt, json_output=False)
        ...
        extracted_data = call_llm("You are a data extraction expert.", extract_prompt, json_output=True)
        ...
        summary = call_llm("You are a financial news summarizer.", summarize_prompt, json_output=False)
        if not summary:
